...
You have finished cleaning your dataset and will now use it to describe the people identified as cases so far and generate a hypothesis to inform the next steps in the outbreak investigation. Note that you should try to have a data analysis plan *before* any data collection. Nevertheless, during the urgency of outbreak investigations, questionnaire development may sometimes occur before you had time to draft a proper analysis plan (as it is the case in this simulated outbreak). Drawing "dummy" tables and graphs will help you precise the type of statistics you will carry out later on.

## 3. Questions/Assignments

If you closed your R project, please open it again and load the clean dataset. We recommend you use the `Copenhagen_clean2.rds` provided by us in EVA. This is just to be sure all needed changes in that file for the code to work have been done (as you may have changed the file in a slightly different way than we have).
...
## 3.3 Describe the outbreak in terms of time.
...
::: {.callout-tip title="Need a little bit of help?" collapse="true"}
`fill` adds an additional variable to be displayed in the plot: `group` is going to determine the fill-colour of our bars. 
`facet_wrap` splits the graph in two: one each for the two levels of `sex`.
:::
...
## 3.4. Describe the outbreak in terms of person.
...
## 3.5 Explore the distribution of all clinical signs (symptoms).
...
## 3.7 Draft the relevant paragraph(s) and table(s) and/or figure(s) in your outbreak report (Methods and results).
